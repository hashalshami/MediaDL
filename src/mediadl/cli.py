...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="mediadl",
        description="Download supported media with MediaDL",
    )
    ...
    try:
        ...
        if args.command == "formats":
            for item in downloader.formats(args.url):
                ...
                print(
                    f"{item.id:>8} {item.ext or '-':>5} "
                    f"{item.resolution or '-':>12} {size!s:>12} "
                    f"{item.vcodec or '-'} / {item.acodec or '-'}"
                )
            return 0
        ...
        def progress(event):
            if event.percent is not None:
                print(
                    f"\r{event.status}: {event.percent:6.2f}%",
                    end="",
                    flush=True,
                )
